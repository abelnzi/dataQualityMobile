        ...
        //console.log(JSON.stringify("==========rows save=========="+JSON.stringify(rows)))
         ...
         if(rows.length>0){
            ...
            await db.transaction(
              tx => {
                tx.executeSql("update orgunit set value = ? where url = ?;", [JSON.stringify(listOrgunit),url]);
              });
            ...
            await db.transaction(
              tx => {
                tx.executeSql("insert into orgunit (url,value) values (?,?)", [url,JSON.stringify(listOrgunit)]);
              });
          }
...
export const getOrgunit = async (url) => new Promise((resolve, reject) => {
  db.transaction(tx => {
    tx.executeSql("select * from orgunit where url=?;", [url],
          function(tx, { rows }) {
            if(rows.length>0) {
...
export const saveDataStore=async (url,dataStore) => {
  ...
  await db.transaction(tx => {
      tx.executeSql("select * from datastore where url=?;", [url], async (_, { rows }) =>{
      //console.log(rows.length)
          ...
          if(rows.length>0){
            ...
            await db.transaction(
                  tx => {
                    tx.executeSql("update datastore set value = ? where url = ?;", [JSON.stringify(dataStore),url]);
                  });
          }else{
            console.log("Je suis ici")
            await db.transaction(
                  tx => {
                    tx.executeSql("insert into datastore (url,value) values (?,?);", [url,JSON.stringify(dataStore)]);
                  });
          } 
...
export const getDataStore = async (url) => new Promise((resolve, reject) => {
  db.transaction(tx => {
      tx.executeSql("select * from datastore where url=?;", [url],
          function(tx, { rows }) {
            if(rows.length>0) {
              resolve(JSON.parse(rows._array[0].value))
            } else{
...
export const saveUser=async (user) => {
  ...
  await db.transaction(tx => {
      tx.executeSql("select * from user where login=? and pwd=? and url=?;", [user.login,user.pwd,user.url], async (_, { rows }) =>{
      console.log("============user rows===========",rows.length)
          ...
          if(rows.length==0){
            ...
            await db.transaction(
                  tx => {
                    tx.executeSql("insert into user (login,pwd,url) values (?,?,?);", [user.login,user.pwd,user.url]);
              });
          }
...
/* export const getUser=async (user,setUser)=>{
  ...
  await db.transaction(tx => {
    tx.executeSql("select * from user where login=? and pwd=? and url=?;", [user.login,user.pwd,user.url], async (_, { rows }) =>{
      //await console.log("========rows user==========="+JSON.stringify(rows))
        if(rows.length>0) {
          console.log("========getUser je suis ici ===========")
          await setUser(user)
        }else{
          console.log("========getUser je suis la ===========")
          setUser(null)
        }
    });
  });
...
export const getUser = async (user) => new Promise((resolve, reject) => {
  ...
  db.transaction(tx => {
    tx.executeSql("select * from user where login=? and pwd=? and url=?;", [user.login,user.pwd,user.url],
          function(tx, { rows }) {
...
export const getLasttUrl = async () => new Promise((resolve, reject) => {
  db.transaction(tx => {
      tx.executeSql("select * from user ;",null,
          function(tx, results) {
            let listUser=results.rows._array
            let taille=listUser.length
            console.log('==========results a voir 2==========',results.rowsAffected)
...
export const getLasttUser = async () => new Promise((resolve, reject) => {
  db.transaction(tx => {
      tx.executeSql("select * from user ;",null,
          function(tx, results) {
            let listUser=results.rows._array
...
export const saveGraphData=async (graphData) => {
  ...
  await db.transaction(tx => {
      ...
      await console.log("============row graphdata==========="+rows.length)

          if(rows.length>0){
            console.log("Je suis la graphdata")
            await db.transaction(
                  tx => {
                    tx.executeSql("update graph_data set value = ? where url=? and uidOu=? and period=? and group_ind=?;", [JSON.stringify(graphData.value),graphData.url,graphData.uidOu,graphData.period,graphData.group_ind]);
                  });
          }else{
            console.log("Je suis ici graphdata")
            await db.transaction(
                  tx => {
                    tx.executeSql("insert into graph_data (url,uidOu,period,group_ind,value) values (?,?,?,?,?);", [graphData.url,graphData.uidOu,graphData.period,graphData.group_ind,JSON.stringify(graphData.value)]);
                  });
          } 
...
export const getGraphData = async (graphData) => new Promise((resolve, reject) => {
  //console.log("======== user in getuser sdb===========",user)
  db.transaction(tx => {
    tx.executeSql("select * from graph_data where url=? and uidOu=? and period=? and group_ind=?;", [graphData.url,graphData.uidOu,graphData.period,graphData.group_ind],
          function(tx, { rows }) {
